...
"""
冒泡排序
冒泡排序是一种基础的排序算法，通过重复遍历数组，交换相邻元素位置来逐步将最大的元素移动到数组的末端。
首先，从数组的起始位置开始，依次比较相邻的两个元素。然后，如果前面的元素大于后面的元素，交换它们的位置。每次遍历结束时，最大的元素被稳定地移动到未排序部分的最后位置。重复这一过程，不断缩小未排序部分的范围，直到整个数组排序完成。
插入排序
插入排序通过逐步扩展一个有序序列的方式来排序数据。
初始条件下，将第一个元素视为已排序。从数组第二个元素开始，把它当作“当前元素”。在已排序序列中，从后向前扫描，将所有比“当前元素”大的元素都向后移动。找到比“当前元素”小或相等的位置后，将“当前元素”插入该位置。继续该过程，直到所有元素都被放入已排序部分。
选择排序
选择排序通过在未排序部分中选择一个最小值来进行排序。
开始时，所有元素都在未排序部分。在未排序部分中遍历，找到最小的元素，然后将这个最小元素和未排序部分的第一个元素交换。增加已排序部分，重复以上过程，直到未排序部分为空。
堆排序
堆排序依赖于堆数据结构来实现排序。
首先，把数组组织成一个最大堆。最大堆的堆顶就是最大值，将其与未排序部分的最后一个元素交换。然后，减小堆的有效大小，并重新调整最大堆。重复交换和调整过程，直至堆的大小减小到1，此时数组被完全排序。
这些排序算法各具优势：冒泡排序和插入排序简单易实现，适合小规模或几乎有序的数据集，选择排序简单直观，而堆排序则较为复杂，但在处理大数据集时表现优异。选择适合的排序算法可以优化程序性能。
"""
